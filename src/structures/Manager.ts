...
export class Manager extends EventEmitter {
  ...
  public searchLocal(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (!node) throw new Error("Nodes arent available to play at this moment. Pls check your lavalink.");

      const res = await node.makeRequest<LavalinkResult>(`/loadtracks?identifier=${query}`) as any;
      if(!res) return reject(new Error("No Query was found."));
  ...
  public search(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (!res) return reject(new Error("No Query was found at this moment."));
  ...
  validatedQuery(queryString:string, node:Node):void {
    ...
    if((Manager.regex.YoutubeMusicRegex.test(queryString) || Manager.regex.YoutubeRegex.test(queryString)) && !node.info.sourceManagers.includes("youtube")) {
      throw new Error("Your Lavalink doesnt not have 'youtube' enabled");
    }
    if((Manager.regex.YoutubeMusicRegex.test(queryString) || Manager.regex.YoutubeRegex.test(queryString)) && !node.info.sourceManagers.includes("yt")) {
      throw new Error("Your Lavalink doesnt not have 'youtube' enabled");
    }
    if((Manager.regex.YoutubeMusicRegex.test(queryString) || Manager.regex.YoutubeRegex.test(queryString)) && !node.info.sourceManagers.includes("scsearch")) {
      throw new Error("Your Lavalink doesnt not have 'soundcloud' enabled");
    }
    ...
    if(Manager.regex.bandcamp.test(queryString) && !node.info.sourceManagers.includes("bandcamp")) {
      throw new Error("Your Lavalink doesnt not have 'bandcamp' enabled");
    }
    ...
    if(Manager.regex.AllSpotifyRegex.test(queryString) && !node.info.sourceManagers.includes("spotify")) {
      throw new Error("Your Lavalink doesnt not have 'spotify' enabled");
    }
    if(Manager.regex.appleMusic.test(queryString) && !node.info.sourceManagers.includes("applemusic")) {
      throw new Error("Your Lavalink doesnt not have 'applemusic' enabled");
    }
    ...
    if(source === "ytmsearch" && !node.info.sourceManagers.includes("youtube"))  {
      throw new Error("Your Lavalink doesnt not have 'youtube' enabled which is required for ytmsearch to work");
    }
    if(source === "spsearch" && !node.info.sourceManagers.includes("spotify"))  {
      throw new Error("Your Lavalink doesnt not have 'spotify' enabled which is required for spsearch to work");
    }
    if(source === "ytsearch" && !node.info.sourceManagers.includes("youtube"))  {
      throw new Error("Your Lavalink doesnt not have 'youtube' enabled which is required for ytsearch to work");
    }
